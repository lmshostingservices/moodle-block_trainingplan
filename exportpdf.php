...
foreach ($cohorts as $cohort) {
    ...
    $hidena = (bool)get_config('block_trainingplan', 'pdf_hide_na');

    foreach ($seq as $r) {
        // Optionally omit N/A units from the export (admin setting, default off).
        if ($hidena && $r->outcome === 'NA') {
            continue;
        }
    ...
    if ($signature) {
        ...
        // TCPDF's writeHTML() cannot render a data-URI <img src>, which is why
        // the signature never appeared. The reliable method is TCPDF's Image()
        // with the '@' prefix, which takes RAW base64 (no data-URI prefix).
        // We strip the leading "data:image/...;base64," and hand TCPDF the raw
        // base64. This is rendered after writeHTML() via $signatureimg below.
        $rawbase64 = $signature;
        if (preg_match('/^data:image\/[a-zA-Z0-9.+-]+;base64,/', $signature)) {
            $rawbase64 = preg_replace('/^data:image\/[a-zA-Z0-9.+-]+;base64,/', '', $signature);
        }
        $signatureimg = $rawbase64;
        $signatureimgdate = $signdate;

        // Render the heading + date via HTML; the image itself is drawn with
        // TCPDF Image() after writeHTML (see below).
        $html .= '
            <br><br>
...
// Draw the signature image (if any) using TCPDF Image() with the '@' raw-base64
// prefix. This is the reliable path — writeHTML() cannot embed data URIs.
if (!empty($signatureimg)) {
    // '@' tells TCPDF the string is raw image data (base64-decoded internally).
    // Place it near the current Y position at the left margin, 50mm wide.
    $pdf->Image('@' . base64_decode($signatureimg), 15, $pdf->GetY(), 50, 0, 'PNG');
}
